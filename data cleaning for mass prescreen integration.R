...
names(UHC)[2:8] <- c("Pre_P1", "Pre_P2", "Pre_P3","Pre_P4","Pre_Med_P5", "Pre_Med_P6", "Pre_Med_P7")
...
write.csv(UHC_w_score,'UHC_noprescreen_2020.csv')
...
UHC_w_score<-read.csv("UHC_noprescreen_2020.csv")
...
write.csv(UHC_long_clean,'UHC_clean_noprescreen_2020.csv')

#lets make sure we're all gucci
UHC_clean<-read.csv("UHC_clean_noprescreen_2020.csv")
...
UHC_final<-UHC_clean[,c(3,11:15,23:35)]
#alrighty, we're ready for data analysis! time to save!
write.csv(UHC_final,'UHC_rdy_for_analysis_noprescreen_2020.csv')

#OK! We need to clean up and paste in our original string of data
prescreen_data<-read.csv("Pretest2020_Full.csv")
#only want RESPONSEID, politparty, free response politparty, str of pol importance, and con/lib spectrum.
prescreen_data_sub<-prescreen_data[,c(9,52:55)]
#lets look @ our work for covid-osf on how to integrate the two datahalfs together!

#ok it looks like we want first the response ID from our data itself
uhc_responseid<-no_prescreen$ResponseId

#use the %in% command to filter entries in a vec
library(tidyverse)
ps_1<-prescreen_data_sub %>% filter(ResponseId %in% uhc_responseid)
...
#
no_prescreen<-read.csv("UHC_rdy_for_analysis_noprescreen_2020.csv")
